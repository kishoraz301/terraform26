...
# ============================================================================
# RESOURCE GROUPS - Create New DMZ Resource Groups
# ============================================================================

resource "azurerm_resource_group" "ne_new" {
  name     = var.source_vms[0].new_rg
  location = var.source_vms[0].location

  tags = {
    Region  = "North Europe"
    Purpose = "DMZ NGINX Proxy Cloned VMs"
  }
}

resource "azurerm_resource_group" "we_new" {
  name     = var.source_vms[2].new_rg
  location = var.source_vms[2].location
...
data "azurerm_virtual_network" "ne_cidmz_vnet" {
  ...
  resource_group_name = azurerm_resource_group.ne_new.name
}

data "azurerm_virtual_network" "we_cidmz_vnet" {
  name                = var.source_vms[2].new_vnet
  resource_group_name = azurerm_resource_group.we_new.name
}
...
data "azurerm_managed_disk" "source_ne_54_os_disk" {
  name                = "SI-RPRXY-VM-54_OsDisk_1_aacc69b09598434fbfb62fccbc93d53f"
...
resource "azurerm_availability_set" "ne_avset" {
  ...
  location            = azurerm_resource_group.ne_new.location
  resource_group_name = azurerm_resource_group.ne_new.name
...
resource "azurerm_availability_set" "we_avset" {
  ...
  location            = azurerm_resource_group.we_new.location
  resource_group_name = azurerm_resource_group.we_new.name
